<template>
  <div class="temperature">
    <div class="header">
      <span class="left">时间比率调整</span>
      <span class="right">调整范围为0~24h</span>
    </div>
...
export default {
  ...
  data() {
    return {
      // arr: [0.4, 0.4, 0.5, 0.6, 0.6, 0.7, 0.7, 0.8, 0.8, 0.9, 
      //       0.9, 1.0, 1.0, 1.1, 1.2, 1.3, 1.2, 1.1, 1.0, 0.9,
      //       0.8, 0.7, 0.6, 0.5],
      arr: [],
      id: null
    }
  ...
  methods: {
    //发丝东请求获取比率
    async getRatio() {
      const result = await reqGetRatio('time');
  ...
  created() {
    this.getRatio();
    // const result = reqAddRatio('time', this.arr);
    // console.log(result);
